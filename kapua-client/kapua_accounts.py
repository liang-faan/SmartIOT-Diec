...
def grant_permissions(account_id, user_id):
    try:
        ...
        # create access info
        body = swagger_client.AccessInfoCreator(userid=user_id,scopeId=account_id)
...
def delete_user_keys(account_id, user_id, key_type='API_KEY'):
    ...
    try:
        ...
        api_response = api_instance.credential_simple_query(account_id, limit=50,
            user_id=user_id)
...
if __name__ == '__main__':
    ...
    if args.command == 'add_acc':
        ...
        if args.command == 'add_user':
            user_id = create_user(account_id=account['id'], name=user['name'])
            print('success create user!')
            grant_permissions(account_id=account['id'], user_id=user_id)
            print('success grant permission!')
            add_user_key(account_id=account['id'], user_id=user_id, password=user['password'])
            print('success add user key')
